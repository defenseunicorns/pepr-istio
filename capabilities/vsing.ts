...
export class K8sAPI {
  ...
  private ingressToVirtualService(ingress: V1Ingress, gateway: string) {
    ...
    ingress.spec.rules.forEach(rule => {
      rule.host && virtualService.spec.hosts.push(rule.host);
  ...
  async createOrUpdateVirtualService(ingress: V1Ingress) {
