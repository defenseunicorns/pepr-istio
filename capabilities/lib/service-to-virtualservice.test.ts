...
import { serviceToVirtualService, extractPort } from "./service-to-virtualservice"; // Replace with the actual import
...
test("extractPort should return undefined when no valid ports", t => {
  ...
  test("extractPort should throw an error for multiple valid ports", t => {
    ...
    t.is(error.message, "Ambiguous ports: More than one TCP port 80 or 443 is specified");
  });
  ...
  test("serviceToVirtualService should throw an error for missing metadata or spec", t => {
    ...
    const error = t.throws(() => serviceToVirtualService(service, "gateway", "hostname"));
